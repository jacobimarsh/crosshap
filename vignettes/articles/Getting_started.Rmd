...
test_HapObject <- run_haplotyping(vcf = vcf,
                LD = LD,
                pheno = pheno,
                metadata = metadata,
                epsilon = epsilon,
...
environment with haplotyping results for all five provided epsilon values. This haplotype 
...
#Phenotype data and parameters used to create haplotype objects
#Add type = 'hap' to ensure it summarizes haplotypes rather than Marker Groups
hap_clustree <- clustree_viz(HapObject = test_HapObject,
                              type = 'hap')
hap_clustree
```
...
MG_clustree <- clustree_viz(HapObject = test_HapObject,
                              type = 'MG')
MG_clustree
```
</details>
...
Hap_viz <- crosshap_viz(HapObject = test_HapObject)
Hap_viz
...
test_HapObject$Haplotypes_MGmin30_E0.6$Indfile
test_HapObject$Haplotypes_MGmin30_E0.6$Hapfile
test_HapObject$Haplotypes_MGmin30_E0.6$Varfile
```
...
top <- build_top_metaplot(HapObject = test_HapObject, epsilon = 0.6, hide_labels = F)

#Next lets build the bottom plot, hiding the guides with "hide_labels = T"
bot <- build_bot_halfeyeplot(HapObject = test_HapObject, epsilon = 0.6, hide_labels = T)
